...
def countdownit(watchlist_path, out_dir, domain=None, key=None, as_html=True):
    """
    ...
    If ``--domain`` (Mailgun domain) and ``--key`` (Mailgun API key) are given, then email the products on sale (if there are any) to the email address listed in the watchlist.
    """
    m.run_pipeline(watchlist_path, out_dir, domain=domain, key=key, 
      as_html=as_html)